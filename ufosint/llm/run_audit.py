"""
Tier B Location Normalization — live dashboard runner.

Provides run_tier_b_with_dashboard() for the full Tier B run with
parallel workers and a live terminal dashboard.

Canonical location for dashboard runner logic, migrated from the
root-level run_audit.py script.
"""
...
from ufosint.llm import audit as audit_mod
...
LOG_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "audit_tier_b.log")
...
def process_batch_for_dashboard(batch_items, model, batch_idx):
    results = audit_mod._process_tier_b_batch(batch_items, model)
    structured = []
...
def run_tier_b_with_dashboard(limit=120000, batch_size=50, workers=15, model=None):
    """Full Tier B run with live dashboard and file logging."""
    if model is None:
        model = audit_mod.DEFAULT_MODEL
    ...
    batch_id = audit_mod._create_batch(conn, "location_normalize", model, len(rows),
    ...
    audit_mod._complete_batch(conn, batch_id, summary)
