"""
Deduplication for the unified UFO sightings database.
...
Canonical location for dedup logic, migrated from the root-level dedup.py.
"""
import argparse
import difflib
import os
...
def main(db_path=None):
    """Run deduplication. Called from thin wrapper or pipeline."""
    if db_path is None:
...
def run_dedup(db_path=None):
    """Run all dedup tiers (no argparse). Called from pipeline."""
    import sys as _sys
    if db_path is None:
        db_path = Config.db_path()
